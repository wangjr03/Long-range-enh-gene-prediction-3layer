...
match <- function(x){
  ...
  ind<- any(link_frame[,1]==as.character(enh_loc[,1])&link_frame[,2]<as.numeric(enh_loc[,3])&link_frame[,3]>as.numeric(enh_loc[,2])&
              link_frame[,1]==as.character(gene_loc[,1])&link_frame[,4]<as.numeric(gene_loc[,3])&link_frame[,5]>as.numeric(gene_loc[,2]))+any(link_frame[,1]==as.character(enh_loc[,1])&link_frame[,4]<as.numeric(enh_loc[,3])&link_frame[,5]>as.numeric(enh_loc[,2])&
                                                                                                                                                link_frame[,1]==gene_loc[,1]&link_frame[,2]<as.numeric(gene_loc[,3])&link_frame[,3]>as.numeric(gene_loc[,2]))
